...
await page.getByText('Week 1', { exact: true }).waitFor();
await page.getByRole('button', { name: 'Previous week' }).click();
await page.getByText('Super Bowl', { exact: true }).waitFor();
await page.getByRole('button', { name: 'Next week' }).click();
await page.getByText('Week 1', { exact: true }).waitFor();
await page.getByRole('button', { name: 'Show Best of Season' }).click();
await page.getByText('Ranked Home 1', { exact: true }).waitFor();
const explorerLog = await getLog();
assert(explorerLog.requests.some((entry) => entry.path === '/api/v1/weeks/2024/regular_season/1'), 'historical regular-season week was not requested');
assert(explorerLog.requests.some((entry) => entry.path === '/api/v1/weeks/2024/postseason/5'), 'circular Super Bowl boundary was not requested');
assert(explorerLog.requests.some((entry) => entry.path === '/api/v1/seasons/2024'), 'Best of Season did not request the selected season');
assert(await page.getByText('Ranked Home 11', { exact: true }).count() === 0, 'historical Best of Season leaked an eleventh game');
assert(await page.getByText('Pro Bowl Home', { exact: true }).count() === 0, 'Pro Bowl leaked into Best of Season');
await page.getByRole('button', { name: 'Select season, currently 2024' }).click();
await page.getByRole('option', { name: '2020' }).click();
await page.getByRole('button', { name: 'Return to weekly games' }).click();
await page.getByText('Week 17', { exact: true }).waitFor();
await page.getByRole('button', { name: 'Previous week' }).click();
await page.getByText('Super Bowl', { exact: true }).waitFor();
const historicalLog = await getLog();
...
assert(await page.getByRole('option', { name: '2024' }).isVisible(), 'mobile season menu omitted 2024');
assert(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth), 'season menu caused narrow horizontal overflow');
await page.keyboard.press('Escape');
assert(await page.getByRole('listbox', { name: 'Available seasons' }).count() === 0, 'Escape did not close the mobile season menu');
await screenshot(page, 'season-explorer-mobile.png');
