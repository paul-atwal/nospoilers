...
await page.getByText('SEA -3.5').waitFor();
...
const reveal = page.getByRole('button', { name: /Reveal score for New England Patriots at Seattle Seahawks/ });
...
await page.getByLabel('Seattle Seahawks score 7').waitFor();
await page.getByLabel('Seattle Seahawks score 10').waitFor({ timeout: 4000 });
await screenshot(page, 'live-revealed-desktop.png');
await page.getByText('Provisional rating').waitFor({ timeout: 4000 });
await page.getByText('Confirmed rating').waitFor({ timeout: 4000 });
assert(await page.getByLabel('Seattle Seahawks score 24').isVisible(), 'revealed score did not continue updating');
...
await page.getByText('Provisional rating').waitFor();
