...
const GameCard: React.FC<GameCardProps> = ({ game, showWeekContext = false }) => {
  ...
  return <article className="bg-neutral-800/40 rounded-xl border border-white/5 overflow-hidden hover:border-white/10 transition-colors shadow-sm"><div className="p-3 md:p-4 flex gap-3 md:gap-4"><div id={`game-${game.id}-teams`} className="flex-1 min-w-0 flex flex-col justify-center py-1"><div className="flex items-center gap-2 text-[10px] font-bold text-neutral-400 uppercase mb-3 tracking-wider">{game.isLive && <span className="relative flex h-2 w-2 mr-1"><span className="relative inline-flex rounded-full h-2 w-2 bg-red-500" /></span>}<span className={game.isLive ? 'text-red-400' : ''}>{game.isUpcoming ? game.dateLabel || 'Date TBD' : game.status}</span><span className="text-neutral-600">•</span>{showWeekContext ? <span className="text-blue-400">{weekInfo.label}</span> : <span className="text-neutral-500">{game.kickoff?.time === 'Kickoff time TBD' ? game.kickoff.time : `${game.dayOfWeek} ${game.kickoffTime}`}</span>}{game.kickoff?.zone && !showWeekContext && <span className="text-neutral-500">{game.kickoff.zone}</span>}{game.broadcaster && !showWeekContext && <span className="text-neutral-500 hidden xs:inline">{game.broadcaster}</span>}</div><div className="flex flex-col gap-3"><TeamRow side="away" team={away} teamScore={game.awayScore} /><TeamRow side="home" team={home} teamScore={game.homeScore} /></div></div><div className="flex flex-col items-center justify-between min-w-[68px] border-l border-white/5 pl-3 md:pl-4 py-1"><div className={`relative w-12 h-12 md:w-14 md:h-14 rounded-full flex flex-col items-center justify-center border-[3px] ${game.isUpcoming ? 'text-neutral-400 border-neutral-700 bg-neutral-800/50' : colorClasses}`}>{game.isUpcoming ? <><span className="text-[9px] uppercase">Odds</span><span className="text-[10px] font-bold text-center">{oddsLabel}</span></> : <><span className="text-[9px] font-bold text-center leading-tight">{rating.label}</span>{score !== null && <span className="font-black text-lg leading-none">{score.toFixed(1)}</span>}</>}</div>{canReveal && <button type="button" onClick={() => { setRevealedGameId(game.id); setIsRevealed((value) => !value); }} className={`mt-1 p-2 rounded-full focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-400 ${revealActive ? 'text-neutral-600 hover:bg-neutral-800' : 'text-blue-400 hover:bg-blue-500/10 hover:text-blue-300'}`} aria-label={revealActive ? `Hide score for ${away.name} at ${home.name}` : `Reveal score for ${away.name} at ${home.name}`} aria-describedby={`game-${game.id}-teams`} aria-pressed={revealActive}>{revealActive ? <EyeOff size={20} /> : <Eye size={20} />}</button>}</div></div></article>;
};
