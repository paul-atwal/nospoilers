...
const weekKey = (week: SeasonWeek | null): string => (
  week ? `${week.season}/${week.phase}/${week.week}` : ''
);
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    void owner.start({
      load: (signal) => apiRef.current!.fetchBootstrap(signal),
      getPollAfterSeconds: (response) => response.pollAfterSeconds,
      ...
      onError: (error) => setBootstrapError(
        getErrorMessage(error, 'Failed to load the NFL week catalogue.'),
      ),
    });
    return () => owner.cancel();
  }, []);
  ...
  useEffect(() => {
    ...
    if (!bootstrap || !selectedWeek || viewMode !== 'weekly') {
      owner.cancel();
      return;
    }
    ...
    if (cached) {
      setWeeklySnapshots((previous) => ({ ...previous, [key]: cached.body }));
    }
    setWeeklyError(null);
    void owner.start({
      load: (signal) => apiRef.current!.fetchWeekSnapshot(selectedWeek, signal),
      getPollAfterSeconds: (response) => response.pollAfterSeconds,
      onLoading: setWeeklyLoading,
      onData: (response: ReadApiResponse<WeekSnapshotResponse>) => {
        setWeeklySnapshots((previous) => ({ ...previous, [key]: response.body }));
        setWeeklyError(null);
      },
      onError: (error) => setWeeklyError(
        getErrorMessage(error, 'Failed to load this week.'),
      ),
    });
    return () => owner.cancel();
  ...
  useEffect(() => {
    ...
    setSeasonError(null);
    void owner.start({
      load: (signal) => apiRef.current!.fetchSeasonSnapshot(season, signal),
      getPollAfterSeconds: (response) => response.pollAfterSeconds,
      onLoading: setSeasonLoading,
      onData: (response: ReadApiResponse<SeasonSnapshotResponse>) => {
        setSeasonSnapshots((previous) => ({ ...previous, [season]: response.body }));
        setSeasonError(null);
      },
      onError: (error) => setSeasonError(
        getErrorMessage(error, 'Could not load season data.'),
      ),
    });
    return () => owner.cancel();
  ...
  useEffect(() => {
    ...
    const onReturn = () => {
      ...
      queueMicrotask(() => {
        queued = false;
        void bootstrapOwner.current.refresh();
        if (viewMode === 'weekly') {
          void weeklyOwner.current.refresh();
        } else {
          void seasonOwner.current.refresh();
        }
      });
    };
  ...
  useEffect(() => () => {
    bootstrapOwner.current.cancel();
    weeklyOwner.current.cancel();
    seasonOwner.current.cancel();
  }, []);
  ...
  if (!bootstrap && bootstrapError) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-[#121212] text-white">
        <AlertCircle className="w-8 h-8 text-red-500" />
        <p className="text-red-200">{bootstrapError}</p>
        <button
          type="button"
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-6 relative">
        <div className="flex justify-between items-end mb-4 px-1">
          <h2 className="text-lg font-bold text-white">
            {viewMode === 'weekly' ? 'Game Rankings' : 'Season Leaders'}
          </h2>
          <button
            type="button"
        ...
        {bootstrapError && (
          <div role="status" className="mb-4 rounded-lg border border-yellow-900/50 bg-yellow-900/10 p-3 text-sm text-yellow-200">
            Catalogue refresh failed; showing the last usable catalogue.{' '}
            <button
              type="button"
              className="underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-400"
              onClick={refreshBootstrap}
            >
              Retry
            </button>
        ...
        {!loading && error && !hasData && (
          <div className="bg-red-900/10 border border-red-900/50 rounded-xl p-6 text-center">
            ...
            <button
              type="button"
              className="mt-3 rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold"
              onClick={() => (
                viewMode === 'weekly'
                  ? weeklyOwner.current.retry()
                  : seasonOwner.current.retry()
              )}
            >
              Retry
            </button>
        ...
        {hasData && error && (
          <div role="status" className="mb-3 rounded-lg border border-yellow-900/50 bg-yellow-900/10 p-3 text-sm text-yellow-200">
          ...
          <div className="flex flex-col gap-3">
            {displayedGames.length === 0 ? (
              <div className="text-center py-20 text-neutral-500 text-sm">
                {viewMode === 'season' ? 'No eligible rated games yet.' : 'No games found.'}
              </div>
            ) : (
              displayedGames.map((game) => (
                <GameCard key={game.id} game={game} showWeekContext={viewMode === 'season'} />
              ))
            )}
          </div>
        )}
